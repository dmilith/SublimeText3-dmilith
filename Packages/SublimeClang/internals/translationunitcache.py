...
def get_clang_libaray():
    clanglib_file = look_for_file(cindex.conf.get_filename(), scriptpath, 3)
    if clanglib_file:
        cindex.Config.set_library_file(clanglib_file)
    try:
        return cindex.conf.lib
    except cindex.LibclangError as e:
        import traceback
        traceback.print_exc()
        error_message(str(e))

clanglib = get_clang_libaray()
...
if cachelib:
    try:
        ...
        import traceback
        traceback.print_exc()
        error_message("Your SublimeClang libcache is out of date. Try restarting ST3 and if that fails, uninstall SublimeClang, restart ST3 and install it again.")
...
def get_cursor_from_position(tu, fn, row, col):
    f = cindex.File.from_name(tu, fn)
    loc = cindex.SourceLocation.from_position(tu, f, row, col)
    return cindex.Cursor.from_location(tu, loc)

def get_template_typerefs(cursor):
        refs = {}
        for child in cursor.get_children():
            if isinstance(child, cindex.Cursor):
                if child.kind == cindex.CursorKind.TYPE_REF:
                    refs[child.displayname] = child.get_resolved_cursor()
        return refs;
...
class Cache:
    ...
    def get_namespace_from_cursor(self, cursor):
        namespace = []
        while cursor and cursor.kind == cindex.CursorKind.NAMESPACE:
    ...
    def find_type(self, data, typename):
        ...
        for ns in namespaces:
            ...
            obj = cache_findType(self.cache, nsarg, nslen, bencode(typename))
            cursor = cindex.Cursor.from_result(obj, None, [self.tu])
            if cursor and not cursor.kind.is_invalid():
                if cursor.kind.is_reference():
                    cursor = cursor.referenced
                break

        if (cursor and not cursor.kind.is_invalid()) or idx == -1:
            return cursor

        # Maybe it's a subtype?
        parent = self.find_type(data, extra)
        if parent and not parent.kind.is_invalid():
    ...
    def solve_member(self, data, typecursor, member, template, template_refs):
        temp = None
        pointer = 0
        if member and not member.kind.is_invalid():
            temp = member.get_returned_cursor()
            pointer = member.get_returned_pointer_level()

            if temp and not temp.kind.is_invalid():
                if temp.kind == cindex.CursorKind.TEMPLATE_TYPE_PARAMETER:
                    ...
                    for child in typecursor.get_children():
                        if child.kind == cindex.CursorKind.TEMPLATE_TYPE_PARAMETER:
                            if child.spelling == temp.spelling:
                                break
                            off += 1
                    if template[1] and off < len(template[1]):
                        ...
                        if isinstance(template[0], cindex.Cursor):
                            temp = template[0]
                        elif isinstance(template[0], str) and template[0] in template_refs:
                            temp = template_refs[template[0]]
                        else:
    ...
    def inherits(self, parent, child):
        if not child or child.kind.is_invalid():
            return False
        if parent == child:
            return True
        ...
        if re.search(r"::$", before):
                    ...
                    if comp:
                        ...
                        for c in comp[0]:
                            if (selfcompletion and not c.baseclass) or \
                                    (inherits and not cindex.AccessSpecifier.from_id(c.access) == cindex.AccessSpecifier.PRIVATE) or \
                                    (cindex.AccessSpecifier.from_id(c.access) == cindex.AccessSpecifier.PUBLIC and \
                                     (
                                        c.static or \
            ...
            if var != None:
                if line > 0 and column > 0:
                    cursor = get_cursor_from_position(self.tu, self.filename, line, column)
                if not cursor or cursor.kind.is_invalid() or cursor.spelling != var:
                    cursor = self.find_type(data, template[0])
                else:
                    pointer = 0  # get the pointer level from the cursor instead
                if cursor and not cursor.kind.is_invalid() and \
                    ...
                    ret = []
                if cursor and not cursor.kind.is_invalid():
                    # cache the template type refs for later usage
                    template_refs = get_template_typerefs(cursor)
                    # It's going to be a declaration of some kind, so
                    # get the returned cursor
                ...
                if clazz != None:
                    ...
                    if cursor and not cursor.kind.is_invalid():
                        ...
                        cursor, template, pointer = self.solve_member(data, cursor, member, template, template_refs)
                        if member and (not cursor or cursor.kind.is_invalid()):
                            ret = []
                if not cursor or cursor.kind.is_invalid():
                    # Is it by any chance a struct variable or an ObjC class?
                    cursor = self.find_type(data, template[0])
                    if not cursor or cursor.kind.is_invalid() or \
                        ...
                        for x in cached_results[0]:
                            if x.cursor.spelling == typename:
                                if x.cursor.kind == cindex.CursorKind.VAR_DECL or \
                                    ...
                                    cursor = cindex.Cursor.from_result(x.cursor, None, [self.tu])
                                    pointer = cursor.get_returned_pointer_level()
                                    cursor = cursor.get_returned_cursor()
                                    if not cursor:
                                        ret = []
            ...
            if cursor and not cursor.kind.is_invalid():
                ...
                while len(tocomplete) and count < 10:
                    if not r or \
                        ...
                        if r and not (r.kind == cindex.CursorKind.TEMPLATE_TYPE_PARAMETER or \
                                             (r.kind == cindex.CursorKind.TYPEDEF_DECL and sum(1 for _ in r.get_children()))):
                            ret = []
                        r = None
                        break
                    count += 1
                    ...
                    if match.group(1) == None and pointer == 0 and r.kind != cindex.CursorKind.OBJC_INTERFACE_DECL:
                        if match.group(2) == "->":
                            ...
                            r, template, pointer = self.solve_member(data, r, comp, template, template_refs)
                            if pointer > 0:
                                pointer -= 1
                            if not comp or comp.kind.is_invalid():
                                ret = []
                        elif match.group(2) == "[]":
                    ...
                    if match.group(1):
                        ...
                        r, template, pointer = self.solve_member(data, r, member, template, template_refs)
                        if not r and member:
                ...
                if r and not r.kind.is_invalid() and (pointer == 0 or r.kind == cindex.CursorKind.OBJC_INTERFACE_DECL):
                        ...
                        if r.kind == cindex.CursorKind.OBJC_INTERFACE_DECL:
                            ...
                            for c in comp[0]:
                                if not c.static and c.cursor.kind != cindex.CursorKind.ENUM_CONSTANT_DECL and \
                                        ...
                                        (cindex.AccessSpecifier.from_id(c.access) == cindex.AccessSpecifier.PUBLIC or \
                                            (selfcompletion and not (c.baseclass and cindex.AccessSpecifier.from_id(c.access) == cindex.AccessSpecifier.PRIVATE))):
                                    disp = c.display
                                    ins = c.insert
                                    for r in replaces:
            ...
            if clazz != None:
                ...
                if c and not c.kind.is_invalid():
                    ...
                    if comp:
                        for c in comp[0]:
                            if not c.static and \
                                    not (c.baseclass and cindex.AccessSpecifier.from_id(c.access) == cindex.AccessSpecifier.PRIVATE):
                    ...
                    if tu2:
                        ...
                        try:
                            for cand in fine_cands:
                                cursor2 = get_cursor_from_position(
                                        tu2.var, cand[0],
                                        cand[1],
                                        cand[2])
                                if cursor2:
    ...
    def get_implementation(self, data, offset, found_callback, folders):
        ...
        try:
            ...
            if len(word_under_cursor) == 0:
                found_callback(None)
                return
            if not cursor or cursor.kind.is_invalid() or cursor_spelling != word_under_cursor:
                if not cursor or cursor.kind.is_invalid():
                    cursor = None
